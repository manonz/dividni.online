...
//const baseUrl ="http://localhost:5066/api";
function Clear() {
...
window.onload = function () {
   ...
   bindDragAndDrop(document.getElementById('sortable-list'));
};
...
function userConfirm(message) {
   return new Promise((resolve) => {
    ...
    const colseAndResolve = (result) => {
     ...
     yesBtn.onclick = () => colseAndResolve(true);
     noBtn.onclick = () => colseAndResolve(false);
   });
 }
 ...
 document.getElementById("useDefaultAnswerSheet").addEventListener("change", function () {
  const answerSheetInput = document.getElementById("answerSheetSvg");
   if (this.checked) {
 ...
 function upload() {
   ...
   formData.append("MathType", document.getElementById("mathType").value);
 ...
 function bindDragAndDrop(sortableList) {
   ...
   const onDragOver = (e) => {
     ...
     if (e.clientY < listRect.top + scrollThreshold) {
       sortableList.scrollTop -= scrollSpeed;
    ...
    fetch(`${baseUrl}/UploadQuestions`, {
     ...
     .then(data => {
       for (let i = 0; i < files.length; i++) {
         ...
         currentCmdLineOrder.push(files[i].name);
       }
 ...
 function deleteQuestionFile(fileName, liElement) {
   ...
   fetch(url, { method: "DELETE" })
     ...
     .then(data => {
       liElement.remove();
       currentCmdLineOrder = currentCmdLineOrder.filter(item => item !== fileName);
     })
     .catch(error => {
       showMessage(error);
       console.error(error);
...
function showMessage(message) {
 ...
 function reorderPreview() {
   ...
   .then(data => {
     ...
     const a4PreviewEmbed = document.getElementById("a4PreviewEmbed");
      previewContainer.style.display = "block";
      a4PreviewEmbed.src = data.mcqPaperPath + "?t=" + Date.now();
      a4PreviewEmbed.onload = function() {
        loader.style.display = "none";
        a4PreviewEmbed.style.display = "block";
      };
